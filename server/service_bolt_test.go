...
import (
	...
	boltdepot "github.com/usaran-devk/scep/v2/depot/bolt"
...
func TestCaCert(t *testing.T) {
	...
	// SCEP service
	svc, err := scepserver.NewService(caCert, key, csrsigner.SignCSRAdapter(scepdepot.NewSigner(depot)))
	if err != nil {
		t.Fatal(err)
	}
...
func createChallengeStore(mode os.FileMode, options *bolt.Options) *challengestore.Depot {
	// Create temporary path.
	f, _ := os.CreateTemp("", "bolt-challenge-")
	f.Close()
	os.Remove(f.Name())
